...
// Supabase Storage 사용
- 이미지 업로드 → 공개 URL 생성
- recommendation에 imageUrl 저장
- 히스토리에서 이미지 표시
```

**구현 위치**:
- `backend/src/storage/storage.service.ts` (이미 있음)
- `app/page.tsx` - 업로드 로직 추가
...
- 🍷 와인 러버 (wine-lover) - 와인 추천 10회
- 🌙 야행성 (night-owl) - 밤 11시 이후 추천 5회
- 🔥 열정적 (passionate) - 일주일 연속 추천
- 💎 프리미엄 (premium-member) - 구독 시작
- 🎯 완벽주의자 (perfectionist) - 즐겨찾기 20개

**구현**:
```typescript
// app/stickers/page.tsx (신규)
- 스티커 그리드 레이아웃
- 진행도 표시
- 해제 애니메이션
```
