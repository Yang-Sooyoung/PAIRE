...
export default function SubscriptionPage() {
  ...
  const [selectedPlanIndex, setSelectedPlanIndex] = useState<number>(1); // Í∏∞Î≥∏Í∞? ?îÍ∞Ñ
  ...
  // Stripe ?§Î°úÍ∞ÄÍ∏??±ÏúºÎ°?storeÍ∞Ä Ï¥àÍ∏∞?îÎêú Í≤ΩÏö∞ Î≥µÍµ¨
  useEffect(() => {
    if (!user || !token) {
      initializeUser();
    }
  ...
  // ÏßÄ??Î∞???Î™®Îìú Í∞êÏ?
  useEffect(() => {
    // IP Í∏∞Î∞ò Í∞êÏ? (Í∞Ä???ïÌôï - VPN??Î∞òÏòÅ)
  ...
  const handleRegisterBilling = async () => {
    ...
    try {
      ...
      setDialogConfig({
        type: 'error',
        title: '?±Î°ù ?§Ìå®',
        description: 'Í≤∞Ï†ú ?òÎã® ?±Î°ù Ï§??§Î•òÍ∞Ä Î∞úÏÉù?àÏäµ?àÎã§.',
      });
      setShowDialog(true);
    } finally {
      setLoading(false);
  ...
  const handleSubscribe = async () => {
    if (!user || !token) {
      setDialogConfig({
        type: 'warning',
        title: isKorean ? 'Î°úÍ∑∏???ÑÏöî' : 'Login Required',
        description: isKorean ? 'Î°úÍ∑∏?∏Ïù¥ ?ÑÏöî???úÎπÑ?§ÏûÖ?àÎã§.' : 'Please login to continue.',
      });
      setShowDialog(true);
      return;
    }
    ...
    try {
      ...
      // Stripe Í≤∞Ï†ú (?¥Ïô∏)
      if (activeRegion.paymentProvider === 'stripe') {
        // Stripe Price ID Îß§Ìïë - ?åÎûúÎ≥ÑÎ°ú ?ïÌôï??Îß§Ìïë
        ...
        // WEEKLY ?òÍ≤ΩÎ≥Ä???ÜÏúºÎ©?MONTHLYÎ°?fallback (Í≤ΩÍ≥† Ï∂úÎ†•)
        if (!stripePriceId) {
          if (selectedPlan.interval === 'WEEKLY') {
            stripePriceId = process.env.NEXT_PUBLIC_STRIPE_PRICE_MONTHLY;
        ...
        // Î∞±Ïóî?úÏóê /api prefix ?ÜÏùå - BASE_URL ?¨Ïö©
      ...
      // ?†Ïä§?òÏù¥Î®ºÏ∏† Í≤∞Ï†ú (?úÍµ≠)
      if (!methodRegistered && !billingKey) {
        setDialogConfig({
          type: 'warning',
          title: 'Í≤∞Ï†ú?òÎã® ?±Î°ù ?ÑÏöî',
          description: 'Í≤∞Ï†ú?òÎã®??Î®ºÏ? ?±Î°ù?¥Ï£º?∏Ïöî.',
        });
        setShowDialog(true);
        setLoading(false);
        return;
      ...
      if (res.data?.subscription || res.data?.success) {
        ...
        setDialogConfig({
          type: 'error',
          title: 'Íµ¨ÎèÖ ?ùÏÑ± ?§Ìå®',
          description: res.data?.message ?? 'Íµ¨ÎèÖ ?ùÏÑ±???§Ìå®?àÏäµ?àÎã§.',
        });
        setShowDialog(true);
      }
    } catch (err: any) {
      if (err?.response?.status === 401) {
        const newToken = await refreshTokenIfNeeded();
        if (newToken) {
          // ?†ÌÅ∞ Í∞±Ïã† ???¨Ïãú?ÑÎäî ?†Ïä§?òÏù¥Î®ºÏ∏†Îß?
      ...
      setDialogConfig({
        type: 'error',
        title: isKorean ? 'Íµ¨ÎèÖ ?îÏ≤≠ ?§Ìå®' : 'Subscription Failed',
        description: err?.response?.data?.message ?? err?.message ?? (isKorean ? 'Íµ¨ÎèÖ ?îÏ≤≠???§Ìå®?àÏäµ?àÎã§.' : 'Failed to create subscription.'),
      });
      setShowDialog(true);
    } finally {
      setLoading(false);
  ...
  const handleCreditPurchase = async (pkg: typeof CREDIT_PACKAGES[0]) => {
    ...
    try {
      ...
      if (activeRegion.paymentProvider === 'stripe') {
        ...
        if (!response.ok) throw new Error('Stripe session ?ùÏÑ± ?§Ìå®');
      ...
      if (!response.ok) {
        if (response.status === 401) {
          ...
          if (newToken) {
            ...
            if (!retryResponse.ok) throw new Error('Íµ¨Îß§ ?ùÏÑ± ?§Ìå®');
            const { orderId, amount, orderName } = await retryResponse.json();
            const tossPayments = await loadTossPayments(process.env.NEXT_PUBLIC_TOSS_TEST_CLIENT_KEY!);
            await tossPayments.requestPayment('Ïπ¥Îìú', { amount, orderId, orderName, successUrl: `${window.location.origin}/credit/success`, failUrl: `${window.location.origin}/credit/fail` });
            return;
          } else {
            router.push('/login');
            return;
      ...
      setDialogConfig({
        type: 'error',
        title: isKorean ? 'Íµ¨Îß§ ?§Ìå®' : 'Purchase Failed',
        description: isKorean ? 'Íµ¨Îß§ Ï§??§Î•òÍ∞Ä Î∞úÏÉù?àÏäµ?àÎã§.' : 'An error occurred during purchase.',
      });
      setShowDialog(true);
    } finally {
      setLoading(false);
    ...
    <div className="min-h-screen bg-background relative overflow-hidden">
      ...
      <div className="bg-card/50 backdrop-blur-sm border-b border-border sticky top-0 z-10">
        <div className="max-w-4xl mx-auto px-4 py-4 flex items-center gap-4">
          ...
          <h1 className={cn(
            ...
            {isKorean ? 'Í≤∞Ï†ú Î∞©Ïãù ?†ÌÉù' : 'Choose Payment Type'}
          </h1>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-4 py-12 relative z-10">
        ...
        >
          ...
          >
            {isKorean ? '?îÑ ?ïÍ∏∞ Íµ¨ÎèÖ' : '?îÑ Subscription'}
          </button>
          <button
          ...
          >
            {isKorean ? '???¨Î†à??Ï∂©Ï†Ñ' : '??Buy Credits'}
          </button>
        </motion.div>
          ...
          >
            <div className="text-center mb-8">
              <p className={cn(
                ...
                {isKorean ? 'Î¨¥Ï†ú?úÏúºÎ°??¥Ïö©?òÏÑ∏?? : 'Unlimited recommendations'}
              </p>
            </div>
            ...
            <div className="bg-card border border-border rounded-xl p-8 mb-8 max-w-2xl mx-auto">
              ...
              <div className="flex gap-3 mb-8">
                {PLANS.map((plan, index) => (
                  ...
                  >
                    <div className="font-semibold text-sm">{plan.interval === 'WEEKLY' ? (isKorean ? 'Ï£ºÍ∞Ñ' : 'Weekly') : plan.interval === 'MONTHLY' ? (isKorean ? '?îÍ∞Ñ' : 'Monthly') : (isKorean ? '?∞Í∞Ñ' : 'Yearly')}</div>
                    ...
                    {plan.interval === 'ANNUALLY' && (
                      <div className="text-xs opacity-80">{isKorean ? '33% ?†Ïù∏' : '33% OFF'}</div>
                    )}
                  </button>
                ))}
              </div>
              ...
              <div className="text-center mb-8 p-6 bg-gold/5 rounded-xl border border-gold/20">
                ...
                <div className={cn(
                  ...
                  {selectedPlan.interval === 'WEEKLY' ? (isKorean ? 'Ï£?1??Í≤∞Ï†ú' : 'Billed weekly') : 
                   selectedPlan.interval === 'MONTHLY' ? (isKorean ? '??1??Í≤∞Ï†ú' : 'Billed monthly') : 
                   (isKorean ? '??1??Í≤∞Ï†ú' : 'Billed annually')}
                </div>
              </div>
                  ...
                  <div className={cn(
                    ...
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="text-sm text-muted-foreground">
                        {isKorean ? '?àÏ†Ñ??Íµ?†ú Í≤∞Ï†ú' : 'Secure International Payment'}
                      </span>
                    </div>
                    <p className="text-xs text-muted-foreground">
                      {isKorean
                        ? 'StripeÎ•??µÌï¥ ?†Ïö©Ïπ¥Îìú, Ï≤¥ÌÅ¨Ïπ¥Îìú, Apple Pay, Google Pay ???§Ïñë??Í≤∞Ï†ú ?òÎã®???¨Ïö©?????àÏäµ?àÎã§.'
          ...
          >
            <div className="text-center mb-8">
              <p className={cn(
                ...
                {isKorean ? '?ÑÏöî??ÎßåÌÅºÎß?Ï∂©Ï†Ñ?òÏÑ∏?? : 'Pay as you go'}
              </p>
              <p className={cn(
                "text-sm text-muted-foreground",
                isKorean && "font-[var(--font-noto-kr)]"
              )}>
                {isKorean ? '?¨Î†à??1Í∞?= Ï∂îÏ≤ú 1?? : '1 Credit = 1 Recommendation'}
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
              {CREDIT_PACKAGES.map((pkg, index) => (
                ...
                >
                  ...
                  <div className="space-y-2 mb-6">
                    <div className="flex items-center gap-2 text-sm text-foreground">
                      ...
                      <span className={isKorean ? "font-[var(--font-noto-kr)]" : ""}>
                        {pkg.credits}{isKorean ? '??Ï∂îÏ≤ú' : ' Recommendations'}
                      </span>
                    </div>
                    {pkg.discount && (
                      <div className="flex items-center gap-2 text-sm text-foreground">
                        ...
                        <span className={isKorean ? "font-[var(--font-noto-kr)]" : ""}>
                          {activeRegion.paymentProvider === 'stripe'
                            ...
                            : `??{pkg.savings?.toLocaleString()}`} {isKorean ? '?àÏïΩ' : 'saved'}
                        </span>
                      </div>
                    )}
                  </div>
                  ...
                  >
                    {loading ? (isKorean ? 'Ï≤òÎ¶¨ Ï§?..' : 'Processing...') : (isKorean ? 'Íµ¨Îß§?òÍ∏∞' : 'Buy Now')}
                  </Button>
                </motion.div>
              ))}
            </div>
            ...
            <div className="max-w-2xl mx-auto">
              <div className={cn(
                ...
                {isKorean ? '?í≥ ?¨Î†à?ßÏ? ?ºÌöå??Í≤∞Ï†úÎ°?Ï¶âÏãú Ï∂©Ï†Ñ?©Îãà?? : '?í≥ Credits are charged immediately with one-time payment'}
              </div>
            </div>
          </motion.div>
        )}
